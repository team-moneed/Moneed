...
import ImageCarousel from '@/6_shared/ui/Carousel/ImageCarousel';
...
export const PostAuthorWithDate = ({
    ...
    author: { profileImage: string; nickname: string };
    createdAt: Date;
}) => {
    return (
        <div className='flex items-center gap-[.6rem] flex-1'>
            <div className='rounded-full overflow-hidden aspect-square sm:w-[2.8rem] w-[2.4rem]'>
                <img src={author.profileImage} alt={author.nickname} className='w-full h-full object-cover' />
            </div>
            <span className='text-[1.4rem] font-normal leading-[140%] text-moneed-black'>{author.nickname}</span>
...
export const PostDropdown = ({ post }: { post: Post }) => {
    ...
    const onEditPost = (e: React.MouseEvent<HTMLButtonElement>) => {
        e.stopPropagation();
        router.push(DYNAMIC_PATH.EDITPOST(post.id));
    };
